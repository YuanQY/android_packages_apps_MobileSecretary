...
public class DbAdapter {
    ...
    public final static String CALL_ALLOW_TABLE_NAME = "call_allow_table";
    // 公用id字段
    ...
    // 来电白名单表字段：白名单号码
    public static final String CALL_ALLOW_PHONE = "call_allow_phone";
    ...
    public Cursor getPhone(String phone, int i) {
        ...
        switch (i) {
            case 1:
                mReturn = mSQLiteDatabase.rawQuery("select * from " + MESSAGE_RUBBISH_TABLE_NAME
                        + " where " + MESSAGE_RUBBISH_PHONE + "=" + "\'" + phone + "\'", null);
                break;
            case 2:
                mReturn = mSQLiteDatabase.rawQuery("select * from " + MESSAGE_FORBID_TABLE_NAME
                        + " where " + MESSAGE_FORBID_PHONE + "=" + "\'" + phone + "\'", null);
                break;
            case 3:
                mReturn = mSQLiteDatabase.rawQuery("select * from " + CALL_RECORD_TABLE_NAME
                        + " where " + CALL_RECORD_PHONE + "=" + "\'" + phone + "\'", null);
                break;
            case 4:
                mReturn = mSQLiteDatabase.rawQuery("select * from " + CALL_FORBID_TABLE_NAME
                        + " where " + CALL_FORBID_PHONE + "=" + "\'" + phone + "\'", null);
                break;
            case 5:
                mReturn = mSQLiteDatabase.rawQuery("select * from " + CALL_ALLOW_TABLE_NAME
                        + " where " + CALL_ALLOW_PHONE + "=" + "\'" + phone + "\'", null);
                break;
        }
        return mReturn;
    }
    ...
    public Cursor getTime(String tableName, String columnName, String recordTime) {
        return mSQLiteDatabase.rawQuery("select * from " + tableName + " where " + columnName
                + " = " + "\'" + recordTime + "\'", null);
    }

    public Cursor getId(long id, String tableName) {
        return mSQLiteDatabase.rawQuery("select * from " + tableName + " where " + TABLE_ID + " = "
                + id, null);
    }
    ...
    public Long getAdd(Call_Allow_Info mInfo) {
        ContentValues mValues = new ContentValues();
        mValues.put(CALL_ALLOW_PHONE, mInfo.getCall_allow_phone());
        long rowId = mSQLiteDatabase.insert(CALL_ALLOW_TABLE_NAME, null, mValues);
        return rowId;
    }
    ...
    public boolean getDel(String table, String column, String phone) {
        return mSQLiteDatabase.delete(table, column + "=?", new String[] {
            phone
        }) > 0;
    }
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        StringBuilder mStringBuilder_5;
        ...
        public void initCreateSQL() {
            ...
            mStringBuilder_5 = new StringBuilder();
            mStringBuilder_5.append("create table ");
            mStringBuilder_5.append(CALL_ALLOW_TABLE_NAME);
            mStringBuilder_5.append("( ");
            mStringBuilder_5.append(TABLE_ID);
            mStringBuilder_5.append(" integer primary key AUTOINCREMENT,");
            mStringBuilder_5.append(CALL_ALLOW_PHONE);
            mStringBuilder_5.append(" text");
            mStringBuilder_5.append(" );");
            Log.d(TAG, "CALL_ALLOW_TABLE_NAME");
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL(mStringBuilder_5.toString());
            Log.d(TAG, mStringBuilder_5.toString());
        }
